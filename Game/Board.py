...
class Board:
    def __init__(self, board_size : int, board_width : int, board_height : int, shift : int, screen : pygame.Surface):
        self.screen = screen
    ...
    def create_Rect(self, row : int, col : int):
        cell_width = (self.board_width - self.nb_padding*self.padding) // self.board_size
        cell_height = (self.board_height - self.nb_padding*self.padding) // self.board_size
    ...
    def draw_board(self):
        ...
        for row in range(self.board_size):
            for col in range(self.board_size):
                pygame.draw.rect(self.screen, self.cell_colors[self.board_values[row, col]], self.cell_rects[row, col])
                if self.board_values[row, col] != 0:
                    
                    font = pygame.font.SysFont('Arial', 40, bold=True)
                    text = font.render(str(self.board_values[row, col]), True, self.text_color)
